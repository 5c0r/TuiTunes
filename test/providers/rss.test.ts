...
const wrap = (items: string) => `<?xml version="1.0"?><rss><channel>${items}</channel></rss>`;
...
describe('parseFeed', () => {
  ...
  test('parses feed with multiple items', () => {
    const items = [1, 2, 3]
      .map((n) =>
        item(
          `<title>Ep ${n}</title>${enclosure(`https://example.com/${n}.mp3`)}<guid>g-${n}</guid><itunes:duration>60</itunes:duration>`,
        ),
      )
      .join('');
